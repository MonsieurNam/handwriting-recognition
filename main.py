...
from app.ocr_engines import OCREngines
from app.processing import run_ocr_pipeline

def main():
    """
    Hàm chính điều phối toàn bộ quy trình:
    1. Thiết lập môi trường.
    2. Tải cấu hình và models.
    3. Xử lý từng ảnh trong thư mục đầu vào.
    4. Lưu kết quả.
    """
    print("=============================================")
    print("=== BẮT ĐẦU CHƯƠNG TRÌNH TRÍCH XUẤT OCR ===")
    print("=============================================")

    # Bước 1: Thiết lập môi trường
    create_directories()
    
    # Bước 2: Tải cấu hình và khởi tạo các engine
    roi_config = load_roi_config()
    if not roi_config:
        print("Dừng chương trình do không tải được cấu hình ROI.")
        return

    # Khởi tạo tất cả các model OCR cần thiết
    # Chỉ khởi tạo một lần và tái sử dụng
    ocr_engines = OCREngines(use_gpu=True)
    ...
    for image_name in os.listdir(INPUT_PATH):
        if image_name.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            extracted_data = run_ocr_pipeline(aligned_image, roi_config, ocr_engines)
